...
<form method="post">
    <div id="d1">
        ...
        CO1:<input type="text" name="ut1" />
        CO2:<input type="text" name="ut2" />
        CO3:<input type="text" name="ut3" />
        CO4:<input type="text" name="ut4" />
        CO5:<input type="text" name="ut5" />
    </div>
    <br><br>
...
if($con->connect_error){
    ...
    if(isset($_POST['ins']))
    {
        $stmt=$con->prepare("insert into $table (reg_id,ut1,ut2,ut3,ut4,ut5) values(?,?,?,?,?,?)");
        $stmt->bind_param("iiiiii",$id,$u1,$u2,$u3,$u4,$u5);
        $id=$_POST['rid'];
        $u1=$_POST['ut1'];
        $u2=$_POST['ut2'];
        $u3=$_POST['ut3'];
    ...
    else if(isset($_POST['upd']))
    {
        ...
        if(isset($_POST['ut4'])){
            $utmark=$_POST['ut4'];
            $sql="update $table set ut4=$utmark where reg_id=$id";
            mysqli_query($con, $sql);
        }
        if(isset($_POST['ut5'])){
            $utmark=$_POST['ut5'];
            $sql="update $table set ut5=$utmark where reg_id=$id";
            mysqli_query($con, $sql);
        }
    }
    ...
    if(isset($_POST['calc'])){
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                $internal_mark=$row["ut1"]+$row["ut2"]+$row["ut3"]+$row["ut4"]+$row["ut5"];
                $internal_mark=$internal_mark/5;
                $id=$row["reg_id"];
    ...
    if(isset($_POST['list_detail'])){
        ...
        if ($result->num_rows > 0) {
            echo "<table id=$t2>";
            echo "<tr><th>ID</th><th>CO1</th><th>CO2</th><th>CO3</th><th>CO4</th><th>CO5</th><th>INTERNAL MARK</th></tr>";
            // output data of each row
            while($row = $result->fetch_assoc()) {
                echo "<tr><td>".$row["reg_id"]."</td><td>".$row["ut1"]."</td><td>".$row["ut2"]."</td><td>".$row["ut3"]."</td><td>".$row["ut4"]."</td><td>".$row["ut5"]."</td><td>".$row["im"]."</td></tr>";
            }
            echo "</table>";
        } 
        else {
    ...
    if($flag==1){
        ...
        if ($result->num_rows > 0) {
            echo "<table id=$t1>";
            echo "<tr><th>ID</th><th>CO1</th><th>CO2</th><th>CO3</th><th>CO4</th><th>CO5</th></tr>";
            // output data of each row
            while($row = $result->fetch_assoc()) {
                echo "<tr><td>".$row["reg_id"]."</td><td>".$row["ut1"]."</td><td>".$row["ut2"]."</td><td>".$row["ut3"]."</td><td>".$row["ut4"]."</td><td>".$row["ut5"]."</td></tr>";
            }
            echo "</table>";
        } 
        else {
