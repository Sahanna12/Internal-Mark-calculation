...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_teacher'])) {
    ...
    if (empty(trim($_POST["tsub"]))) {
        ...
        $sql_check_subject = "SELECT subject_id FROM subjects WHERE subject_name = ?";
        if ($stmt_check_subject = $conn->prepare($sql_check_subject)) {
            // Bind variables to the prepared statement as parameters
            $stmt_check_subject->bind_param("s", $param_tsub);

            // Set parameters
            $param_tsub = $tsub;

            // Attempt to execute the prepared statement
            if ($stmt_check_subject->execute()) {
                // Store result
                $stmt_check_subject->store_result();

                if ($stmt_check_subject->num_rows == 0) {
                    // Subject doesn't exist, insert it into subjects table
                    $sql_insert_subject = "INSERT INTO subjects (subject_name) VALUES (?)";

                    if ($stmt_insert_subject = $conn->prepare($sql_insert_subject)) {
                        // Bind variables to the prepared statement as parameters
                        $stmt_insert_subject->bind_param("s", $param_tsub);
                        ...
                        if ($stmt_insert_subject->execute()) {
                            ...
                            echo "Error: " . $sql_insert_subject . "<br>" . $conn->error;
                        }
                ...
                echo "Error: " . $sql_check_subject . "<br>" . $conn->error;
            }
        ...
        // Check if table exists for the subject
        $sql_check_table = "SHOW TABLES LIKE '$tsub'";
...
if ($result_check_table->num_rows == 0) {
    ...
    if ($conn->query($sql_create_table) === TRUE) {
        /*$success_msg .= "<br>Table '$tsub' created successfully";*/
    } else {
        ...
        // Prepare an insert statement for teacher
        ...
        if ($stmt = $conn->prepare($sql_teacher)) {
            ...
            // Set parameters
            $param_tid = $tid;
            $param_tpass = $tpass; // Hash the password
            $param_tsub = $tsub;

            // Attempt to execute the prepared statement
            if ($stmt->execute()) {
                $success_msg .= "<br>New teacher record created successfully";
            } else {
